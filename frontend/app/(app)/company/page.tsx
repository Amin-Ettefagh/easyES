...
export default function CompanyPage() {
  ...
  return (
    <div>
      ...
      <div className="grid cols-2 mb"><div className="card company-panel"><div className="section-head"><div><span className="section-kicker">Tenancy</span><h3>Your companies</h3></div><span className="count-pill">{organizations.length}</span></div>{organizations.map((company) => <div className="company-summary-row" key={company.uuid}><span><strong>{company.name}</strong><small>{company.type.replaceAll("_", " ")}</small></span><span>{company.project_count} workspaces</span></div>)}</div><div className="card company-panel"><div className="section-head"><div><span className="section-kicker">Delivery</span><h3>Workspaces</h3></div><span className="count-pill">{workspaces.length}</span></div>{workspaces.length ? workspaces.map((workspace) => <div className="company-summary-row" key={workspace.uuid}><span><strong>{workspace.name}</strong><small>{workspace.idea || "Project workspace"}</small></span><span>{workspace.workflow_count || workspace.workflows?.length || 0} workflows</span></div>) : <div className="muted">Create a workspace from Dashboard.</div>}</div></div>
